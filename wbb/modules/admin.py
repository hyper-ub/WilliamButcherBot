...
from wbb import OWNER_ID, SUDO_USERS, app
...
async def purge(client, message):
    ...
    if "can_delete_messages" in permissions or user_id in SUDO_USERS:
...
async def kick(_, message):
    try:
        ...
        if "can_restrict_members" in permissions or from_user_id in SUDO_USERS:
            if len(message.command) == 2:
                username = message.text.split(None, 1)[1]
                if (await app.get_users(username)).id in SUDO_USERS:
...
async def delete(_, message):
    ...
    try:
        ...
        chat_id = message.chat.id
        permissions = await member_permissions(chat_id, from_user_id)
        if "can_delete_messages" in permissions or from_user_id in SUDO_USERS:
...
async def promote(_, message):
    try:
        ...
        chat_id = message.chat.id
        permissions = await member_permissions(chat_id, from_user_id)
        if "can_promote_members" in permissions or from_user_id in SUDO_USERS:
            if len(message.command) == 2:
                username = message.text.split(None, 1)[1]
...
async def pin(_, message):
    ...
    try:
        ...
        chat_id = message.chat.id
        permissions = await member_permissions(chat_id, from_user_id)
        if "can_pin_messages" in permissions or from_user_id in SUDO_USERS:
